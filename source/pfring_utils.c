...
int pfring_parse_pkt(u_char *pkt, struct pfring_pkthdr *hdr, u_int8_t level /* L2..L4, 5 (tunnel) */,
  ...
  if (hdr->extended_hdr.parsed_pkt.eth_type == 0x8864 /* PPPOE Session */ || hdr->extended_hdr.parsed_pkt.eth_type == 0x8863 /* PPPOE Discovery */) {
    displ += PPPOE_HDR_LEN;
    unsigned char *ppp = pkt + hdr->extended_hdr.parsed_pkt.offset.eth_offset + displ + sizeof(struct ethhdr);
    short pppproto = ntohs(*(short*)ppp);
    if (pppproto == 0x0021 /* IP, Internet Protocol. */) {
